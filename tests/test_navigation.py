...
def test_auction_has_live_intelligence_personal_war_room_and_owner_insights() -> None:
    ...
    assert '<aside class="auction-values-column" aria-label="Team money"><section' in auction
    assert (
        ".auction-values-column .budget-grid{height:auto;max-height:none;"
        "min-height:0;overflow-y:auto}"
        in css
    )
    assert "--auction-panel-height:calc(65vh + 9rem)" in css
    assert ".auction-viewer-workspace .auction-main-column>.players>.table-wrap" in css
    assert 'id="auction-name-filter"' not in auction
    ...
    assert "activity.classList.toggle('next-nominator'" in script
    assert ".auction-activity-strip.current-nominator" in css
    assert ".auction-activity-strip.next-nominator" in css
